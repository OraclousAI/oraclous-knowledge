...
* [application-gateway-service](application-gateway-service.md) (port 8006) — **Real (R3.5-complete) reverse-proxy edge + R6 hardening code-complete (all 9 slices; §22 sign-off pending):** longest-prefix route table → streaming proxy, edge JWT termination + identity forwarding (anti-spoof), CORS, health aggregation, the live **ORA-37 error envelope**, plus the **versioned public OpenAPI contract** (`/v1/openapi.json` + `openapi-diff-gate`, Slice 1), the **edge rate-limit + request-size guard** (Redis-backed; fail-open limit / fail-closed size, Slice 2), its **own org-scoped Postgres + the integration-key store/validator** (Slice 3, ADR-019 — fail-closed `oak-`/`oag-` key auth minting a SERVICE_ACCOUNT principal), the **published-agent surface + integration-key CRUD** (Slice 4 — publish/mint/rotate/revoke + binding-enforced GET-metadata/invoke), **per-key / per-origin CORS** for the agent widget plane (Slice 5 — fail-closed, credential-free), **member console chat** (Slice 6 — persisted, private-to-creator, riding the harness), **webhook ingress** (Slice 7 — the first public, signature-gated door firing the engine; the signing secret lives in the broker, ADR-008), an **MCP server** (Slice 8 — published agents as tools for external MCP clients, integration-key auth), and **sole-ingress** (Slice 9 — the upstreams are internal-only; the gateway is the sole host-reachable application surface). All 9 R6 slices shipped; the MCP *client*/import is a deferred follow-on.
