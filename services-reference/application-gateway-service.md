...
* **Published-agent public surface + integration-key CRUD** (Slice 4 — `GET /v1/agents/{slug}` + invoke, plus publish/list/rotate/revoke; the key store + the inbound validator already shipped in Slice 3, and the per-key binding is enforced here)
