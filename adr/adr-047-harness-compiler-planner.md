...
**The posture is fixed as compile-and-run by default; plan-approval is an opt-in *setting*, not a precondition.** This is the #391 lock §4 CUT decision, which **overrides** the capability-design §12.2 recommendation (propose-then-approve default). *Surfaced as an open founder decision below* (capability-design §12 #2 recommends the opposite default), but the epic's CUT list governs: "an experienced conductor compiles-and-runs; do not build a required approval gate into the compile flow."
...
## Open founder decisions (for Reza)

| # | Decision | Recommendation | Status |
| --- | --- | --- | --- |
| **1 — Planner autonomy** (capability-design §12 #2) | Auto-emit-and-run vs. propose-then-approve as the **default**. | Capability-design §12.2 recommends **propose-then-approve default + opt-in autonomous**. The **#391 lock §4 CUT list overrides** to **compile-and-run default + opt-in approval** ("plan-approval is a setting, not a precondition"). This ADR adopts the epic's CUT decision (decision 6). **Flagging the divergence for Reza's explicit ratification** — if Reza prefers the §12.2 conservative default, decision 6 inverts (approval-on by default, autonomous opt-in) with no other change. | **Needs Reza ratification.** |
...
- **The platform gains its primary on-ramp.** A user states an objective in English and Oraclous builds the team — the missing half of the Goal. P1 (planning) moves from `Built? = None` to shipped, ADR'd.
...
- [ADR-034 — Adoption-First Import](adr-034-adoption-first-import.md) / [ADR-045 — Prose-showrunner → DAG import adapter](adr-045-prose-showrunner-dag-import-adapter.md) — the *secondary* (import) on-ramp; the shared `assemble_team()`/`ImportReport` dry-run validator (one validator, two on-ramps); the O8 dry-run + flag-not-guess discipline.
