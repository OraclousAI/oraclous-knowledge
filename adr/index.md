...
* **Numbering is monotonic.** ADR-052 is the next number for NEW decisions (ADR-031–051 taken; ADR-051 workspace listing vs the org-scoped read gate, Accepted); numbers are not reused even if an ADR is rejected. (ADR-022 is a **ported legacy ADR** kept at its original number for traceability.)
