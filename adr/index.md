...
| ADR-052 | [An App-Descriptor Layer Maps a Generated App's Shape onto a Team Run's Inputs — a separate, OHM-style versioned YAML file (not the run manifest) declares an app's own input/output shape and its mapping onto the run's declared `inputs`, authored up front or auto-derived from an already-run team; "this input is a hypothesis, not a premise" becomes a general platform concept; the validation desk ships a one-off field now and migrates later (Contract oraclous-backend#845, first consumer #846)](adr-052-app-descriptor-layer-for-generated-apps.md) | Accepted — decision 3 realized 2026-08-19 (#846) | 2026-08-18 |
| ADR-053 | [A Structured Member Answer Is Enforced Mechanically: a Per-Run Schema, a Member-Level Declaration, and a Platform-Minted Receipt — `OHMCapability.resolved_schema` carries a per-run, per-org JSON Schema override; `OHMMember.answer_from_tool` declares that a tool call IS the member's answer; when the loop terminates on that call, the platform mints the `driving_signals` receipt from its own dispatch record — a deliberate, bounded exception to the #642/#743/#781 trust-narrowing line (oraclous-backend#901, forced by the live failure on #1043)](adr-053-structured-member-answer.md) | Proposed 2026-09-12 — decisions ruled, CTO acceptance pending; not yet built | 2026-09-12 |
...
> **ADR-022** is **ported from the legacy `develop` branch @ `84152635de05c105765cfe6b631bb5ba81f2f4aa` (TASK-237)** and kept at its **original legacy number** for traceability to the binding spec. It is the binding ingestion specification for the R3.5 knowledge-graph-service (service #1). The next NEW number is **ADR-054** (ADR-031–053 taken; ADR-053 structured member answer, Accepted 2026-09-12; 022 is the ported legacy ADR).
...
* **Numbering is monotonic.** ADR-054 is the next number for NEW decisions (ADR-031–053 taken; ADR-053 structured member answer, Accepted); numbers are not reused even if an ADR is rejected. (ADR-022 is a **ported legacy ADR** kept at its original number for traceability.)
