...
* **Numbering is monotonic.** ADR-035 is the next number for NEW decisions (ADR-031/032/033 + ADR-034 taken for the R7 product-loop); numbers are not reused even if an ADR is rejected. (ADR-022 is a **ported legacy ADR** kept at its original number for traceability.)
