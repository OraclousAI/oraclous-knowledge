...
> **ADR-022** is **ported from the legacy `develop` branch @ `84152635de05c105765cfe6b631bb5ba81f2f4aa` (TASK-237)** and kept at its **original legacy number** for traceability to the binding spec. It is the binding ingestion specification for the R3.5 knowledge-graph-service (service #1). The next NEW number is **ADR-042** (ADR-031–041 taken; ADR-041 E6 Artifact home + pluggable sinks + graph universal index, Accepted 2026-06-25; 022 is the ported legacy ADR).
...
* **Numbering is monotonic.** ADR-042 is the next number for NEW decisions (ADR-031–041 taken; ADR-041 E6 artifact home + pluggable sinks + graph universal index, Accepted); numbers are not reused even if an ADR is rejected. (ADR-022 is a **ported legacy ADR** kept at its original number for traceability.)
