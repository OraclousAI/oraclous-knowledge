...
* **Numbering is monotonic.** ADR-044 is the next number for NEW decisions (ADR-031–043 taken; ADR-043 conductor + Flow-6 consciousness Learn loop, Proposed); numbers are not reused even if an ADR is rejected. (ADR-022 is a **ported legacy ADR** kept at its original number for traceability.)
