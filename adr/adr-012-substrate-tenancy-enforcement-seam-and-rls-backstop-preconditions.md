...
> **✅ Implementation status (as-built, updated 2026-06-17 — §2 RLS backstop NOW REALIZED).** The §1 enforcement seam exists (`oraclous_substrate.access` — the `scoped_*` ops + `bind_organisation_guc`/`scoped_pg_connection`), and the **§2 RLS backstop is now realized across all 7 backend services** (epic [`oraclous-backend#353`](https://github.com/OraclousAI/oraclous-backend/issues/353) closed; design in [ADR-030](adr-030-realize-postgres-rls-backstop.md)). Every service connects at runtime as the `NOSUPERUSER NOBYPASSRLS` `oraclous_app` role with `ENABLE`+`FORCE ROW LEVEL SECURITY` + an org-isolation policy on every org-scoped table — **27 forced-RLS tables**, proven by data-layer isolation tests + live smoke. The async GUC-binding mechanism this ADR's §2 anticipated is `oraclous_substrate.access_async` (`install_org_guc_guard` binds `app.current_organisation_id` transaction-locally per request via `org_scope`); the preconditions (`NOSUPERUSER`/`NOBYPASSRLS` role, transaction-local GUC) are met. **App-layer org-scoping remains the primary control; RLS is now the realized defense-in-depth second line** (it was app-layer-only when the grade-A WP-5 audit corrected the earlier over-claim, 2026-06-17 same day). §1–§3 below are the realized shape. *(Prior note, for the record: as of the WP-5 audit earlier on 2026-06-17 no migration enabled RLS and the runtime ran as superuser — the ORA-16 "shipped the FORCE'd RLS policy" Context line had not borne out; #353 closed that gap.)*
