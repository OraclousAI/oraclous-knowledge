...
>
> **rev7 (2026-08-14, Parham Davari).** Two decisions ([`oraclous-backend#800`](https://github.com/OraclousAI/oraclous-backend/issues/800), ruled together with [`#786`](https://github.com/OraclousAI/oraclous-backend/issues/786)). Both follow from one sentence: **a missing value is not evidence of an origin.** §CITE had made a surface say where content came from; it had not said what a surface does when it does not know, and two of them independently chose to guess `upload`.
>
> 17. **Origin and producer are two fields, and neither is derived from the other's absence.** A surface that shows where an artifact came from reads `source_system`. A surface that shows which agent made it reads `producer_kind`, and shows nothing when that is null. **Stated reason:** the Artifacts tab rendered a null `producer_kind` as the word "Upload", which is true for a manual upload and false for every connector-triggered ingest, because neither has an agent producer. One column was answering two questions and had to be wrong about one of them. Widening `producer_kind` to cover connector reads was rejected: it carries a producer reference — the run, the member role, the execution — that a connector read has none of, and the same field gates agent writes at ingest.
> 18. **`source_system` may be absent on an artifact record, and absent is stated rather than defaulted.** A record whose `SourceRef` is missing or unparseable reports `null`, and a surface renders that as *not recorded*. It is never reported as `upload`. **Stated reason:** `upload` is a claim about who brought the content, and a record that predates the provenance columns supports no such claim. This is the same rule the console already follows for a null citation, where it says the record was ingested before source identity was captured rather than leaving a blank. A genuine upload is unaffected: it carries a real `SourceRef` and still reports `upload`.
...
### Where origin surfaces outside an answer — the artifact record (rev7)

A citation is not the only place a user asks where content came from. The **Artifacts list** (`GET /v1/artifacts`) describes the same underlying documents from the other side: not "what did this answer cite" but "what is in this workspace, and where did it come from". The two must agree about one record, so the artifact summary carries the origin §CITE already mints.

```jsonc
// ArtifactSummary — the two provenance fields, and what each one answers
{
  "source_system": "github",     // WHERE IT CAME FROM. The minted SourceRef's slug.
                                 // null when the record has no parseable source (rev7).
  "producer_kind": null          // WHICH AGENT MADE IT. null when no agent did.
}
```

**They answer different questions, and a surface never derives one from the other (decision 17).** `producer_kind` distinguishes agent-produced content from human-produced content; it carries a producer reference — the team run, the member role, the execution — and it is the field an agent write declares at ingest. A connector-triggered ingest and a manual upload are both human-triggered, so both leave it null. Reading that null as "upload" is therefore correct for one of the two doors and wrong for the other, which is exactly what the Artifacts tab did before rev7. Origin comes from `source_system`; the producer column shows nothing when there is no producer.

**Absent origin is stated, not defaulted (decision 18).** `source_system` is nullable on an artifact record. A row that predates the provenance columns, or one whose stored `source` no longer parses as a `SourceRef`, reports `null` — never `upload`. A surface renders it the way the console already renders a null citation: an explicit note that the source was not recorded, so it cannot be traced back to an original document. Blank space would read as a loading state and hide the gap; `upload` would assert something nobody verified.

This is the same discipline as the `revision` rule above, applied one level out. There, a source with no version of its own gets an honest `content_hash` rather than a borrowed version number. Here, a record with no source gets an honest absence rather than a borrowed origin.

**The reserved value `agent` matters most on this surface.** An artifact a member wrote into a workspace reports `source_system: "agent"` (rev4 decision 7), so a reader scanning the list can tell platform-authored content from documents a person brought, without opening anything. Recording it as `upload` concealed exactly that distinction, and it is tracked as [`oraclous-backend#786`](https://github.com/OraclousAI/oraclous-backend/issues/786).
