...
> **R3.5 status note (2026-06-04) — the layer split is the target, not the shipped reality.** The four layers below describe where each capability *belongs*. As of this date, that mapping is **not** what runs. R2/R3 (and R2's Layer 2 capability-registry) shipped **hollow** — stub endpoints, `raise NotImplementedError`, a `GraphNodeService` stub class defined inside a route file — while the real Layer 1 (knowledge-graph build + retrieve) and Layer 2 (capability registry, tools, connectors) logic still lives undeleted in `oraclous-backend/oraclous-core-service/` (~6,300 LOC dead/undeleted). Layer 1 identity is also incomplete: the shipped auth dropped human/email/OAuth/org management. **R3.5 executes this migration for real, per service**, in graph-first order — (1) knowledge-graph-service → (2) knowledge-retriever-service → (3) a new identity/org service (users + email + OAuth + orgs/members/roles/invites; orgs leave the graph service) → (4) credential-broker-service → (5) capability-registry + tools + connectors (ported from `oraclous-core-service`, which is then **salvaged-then-deleted**, human-gated) → (6) application-gateway. Until the gateway (Layer 4) exists, services are reached **directly by host IP:port** (legacy parity). The canonical authority is **ORAA-4 operating-contract (§21–§23)**; when this section and ORAA-4 diverge, ORAA-4 wins. The original layer descriptions below are unchanged.
