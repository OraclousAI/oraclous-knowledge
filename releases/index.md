...
>
> **Re-planned releases shipped (as of 2026-06-10):** R4 (harness-runtime) ✅ · R5 (execution-engine) ✅ · **R6 (gateway hardening) ✅ Released — §22-signed-off** ([brief](r6-gateway-hardening-brief.md); 9 slices, the gateway is now the sole external surface: OpenAPI · rate-limit/size · integration-key store · published agents+keys · per-key CORS · member chat · webhook ingress · MCP server · sole-ingress). **Active: [R7-SEC — external-launch security hardening](r7-security-hardening-brief.md)** (the re-planned R8 security pass — an org-admin roles floor + per-org KMS envelope encryption + the cheap R6 follow-ups; invite-only launch model, KMS stages post-launch). The original R7/R8 phase rows below stay superseded.
